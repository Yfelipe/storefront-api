...
export class ProductStore {
    ...
    async show(id: string): Promise<Product> {
        try {
            ...
            throw new Error(`Sorry we had an issue in finding your product, error: ${err}`)
        }
    }
    ...
    async create(product: Product): Promise<Product> {
        try {
            ...
            throw new Error(`Sorry we had an issue in adding the product, error: ${err}`)
        }
    }
    ...
    async productsByCategory(category: string): Promise<Product[]> {
        try {
            ...
            throw new Error(`Sorry we had an issue finding products by categories, error: ${err}`)
        }
    }
}
